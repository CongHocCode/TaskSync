...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Helper lấy base URL tự động của hệ thống
    const getBaseUrl = () => {
        const pathParts = window.location.pathname.split('/');
        if (pathParts[1] === 'TaskSync') {
            return window.location.origin + '/TaskSync';
        }
    ...
    function updateTaskStatus(taskId, status) {
        ...
        .then(data => {
            if (data.success) {
                console.log(`[TaskSync] Đã cập nhật thành công Task ${taskId} sang ${status}`);
            } else {
    ...
    function openTaskDetailModal(taskId, card) {
        ...
        const descTextarea = modalElement.querySelector('.mb-4 textarea:not(#modalTaskTitle)');
        const assigneeSelect = modalElement.querySelector('select:not(#modalStatusSelect)');
        const githubInput = modalElement.querySelector('input[type="text"][placeholder*="github" i]') || modalElement.querySelector('.col-lg-4 input[type="text"]');
        const projectHeader = modalElement.querySelector('.modal-header .text-muted');
        ...
        if (subtaskList) subtaskList.innerHTML = '<div class="text-center py-2"><span class="spinner-border spinner-border-sm text-secondary" role="status"></span></div>';
        ...
        .then(task => {
            ...
            if (assigneeSelect) {
                // Ánh xạ tên người gán với option text của Quyền
                const assigneeValueMap = {
                    'Alex': 'Alex (AL)',
                    'Sarah': 'Sarah (SA)',
                    'Quyen': 'Quyen (QU)',
                    'Marcus': 'Marcus (MA)'
                };
                assigneeSelect.value = assigneeValueMap[task.assignee_name] || assigneeSelect.options[0].value;
            }
            if (githubInput) {
            ...
            if (task.subtasks && task.subtasks.length > 0) {
                ...
                if (subtaskBadgeCount) subtaskBadgeCount.innerText = `${completedCount} / ${task.subtasks.length}`;
                if (subtaskProgressBar) subtaskProgressBar.style.width = `${(completedCount / task.subtasks.length) * 100}%`;
            } else {
                subtasksHtml = '<div class="text-muted small py-1">Không có sub-task nào.</div>';
                if (subtaskBadgeCount) subtaskBadgeCount.innerText = '0 / 0';
                if (subtaskProgressBar) subtaskProgressBar.style.width = '0%';
            }
        ...
        .catch(err => {
            ...
            if (descTextarea) descTextarea.value = "Không thể kết nối đến server để lấy mô tả.";
            if (subtaskList) subtaskList.innerHTML = '<div class="text-danger small py-1">Lỗi tải danh sách sub-tasks.</div>';
        });
    }
